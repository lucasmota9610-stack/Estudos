...
function load(){const raw=localStorage.getItem(STORAGE_KEY);
  if(!raw){const materias=DEFAULT_MATERIAS.map((nome,i)=>({id:String(i+1),nome,historico:[]})); const metaSemanal=14; save({materias,metaSemanal}); return {materias,metaSemanal};}
  try{return JSON.parse(raw);}catch{return {materias:[],metaSemanal:14};}}
function save(s){localStorage.setItem(STORAGE_KEY, JSON.stringify(s));}

function render(){
  const s=load();
  document.getElementById('hoje').textContent=new Date().toLocaleDateString();
  document.getElementById('meta').value=s.metaSemanal;
  ...
  document.getElementById('bar').style.width=Math.min(100, Math.round((totalSemana/s.metaSemanal)*100)||0)+'%';
...
document.getElementById('btnAdd').addEventListener('click',()=>{
  const inp=document.getElementById('novaMateria'); const nome=inp.value.trim(); if(!nome) return;
  update(s=>{s.materias.unshift({id:String(Date.now()),nome,historico:[]});}); inp.value='';
});
document.getElementById('meta').addEventListener('change',e=>{
